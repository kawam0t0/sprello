...
export function TimelineView({ board }: TimelineViewProps) {
  ...
  const dateRange = useMemo(() => {
    if (timelineItems.length === 0) {
      ...
      return {
        ...
        end: new Date(now.getFullYear(), now.getMonth() + 10, 1),
        months: [],
      }
    }
    ...
    timelineItems.forEach((item) => {
      const dates = [item.startDate, item.openDate].filter(Boolean) as Date[]
      dates.forEach((date) => {
    ...
    const start = new Date(earliestDate.getFullYear(), earliestDate.getMonth() - 2, 1)
    const end = new Date(latestDate.getFullYear(), latestDate.getMonth() + 14, 1)
  ...
  const dateToPosition = (date: Date): number => {
    ...
    const daysSinceStart = (date.getTime() - dateRange.start.getTime()) / (1000 * 60 * 60 * 24)
  ...
  const handleDateUpdate = async () => {
    ...
    setIsUpdating(true)
    try {
      const updateData = editingDate.type === "start" ? { start_date: newDate } : { open_date: newDate }
        ...
        <div className="min-w-[1200px] p-6">
            ...
            <div className="flex-1">
              <div className="flex bg-gradient-to-r from-teal-50 to-cyan-50 rounded-t-lg overflow-hidden border-b-2 border-teal-400">
                {dateRange.months.map((month, idx) => (
                  <div key={idx} className="flex-1 text-center py-3 border-l border-teal-200 first:border-l-0">
                  ...
                  <div className="w-72 flex-shrink-0 pr-4">
                    <div className="p-3 rounded-lg shadow-sm border-l-4 border-gray-400 bg-white hover:shadow-md transition-shadow">
                      ...
                      <div className="flex flex-wrap gap-1.5 mb-2">
                        <span className="text-xs bg-gray-100 px-2 py-1 rounded-full font-medium">{item.listTitle}</span>
                      </div>
                  ...
                  <div className="flex-1 relative h-24 bg-gray-50 rounded-lg border border-gray-200">
                    {item.paymentDates.map((payment, payIdx) => (
                      ...
                      >
                        ...
                        <div className="absolute -top-10 left-1/2 transform -translate-x-1/2 whitespace-nowrap bg-gray-800 text-white text-xs px-3 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20 shadow-lg">
                          着金{payIdx + 1}:{" "}
                          {payment.start.toLocaleDateString("ja-JP", { month: "numeric", day: "numeric" })} 〜{" "}
                          {payment.end.toLocaleDateString("ja-JP", { month: "numeric", day: "numeric" })}
                        </div>
                      </div>
                    ))}
                    ...
                    {item.startDate && item.startEndDate && (
                      ...
                      >
                        ...
                        <div className="absolute -top-10 left-1/2 transform -translate-x-1/2 whitespace-nowrap bg-gray-800 text-white text-xs px-3 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20 shadow-lg">
                          工事期間: {item.startDate.toLocaleDateString("ja-JP", { month: "numeric", day: "numeric" })}{" "}
                          〜 {item.startEndDate.toLocaleDateString("ja-JP", { month: "numeric", day: "numeric" })}
                    ...
                    {item.setupStartDate && item.setupEndDate && (
                      ...
                      >
                        ...
                        <div className="absolute -top-10 left-1/2 transform -translate-x-1/2 whitespace-nowrap bg-gray-800 text-white text-xs px-3 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20 shadow-lg">
                          設営: {item.setupStartDate.toLocaleDateString("ja-JP", { month: "numeric", day: "numeric" })}{" "}
                          〜 {item.setupEndDate.toLocaleDateString("ja-JP", { month: "numeric", day: "numeric" })}
                        </div>
                      </div>
                    )}
                    ...
                    {item.openDate && item.openEndDate && (
                      ...
                      >
                        <div className="absolute -top-10 left-1/2 transform -translate-x-1/2 whitespace-nowrap bg-gray-800 text-white text-xs px-3 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20 shadow-lg">
                          OPEN期間: {item.openDate.toLocaleDateString("ja-JP", { month: "numeric", day: "numeric" })} 〜{" "}
                          {item.openEndDate.toLocaleDateString("ja-JP", { month: "numeric", day: "numeric" })}
                    ...
                    {item.openFollowStartDate && item.openFollowEndDate && (
                      ...
                      >
                        ...
                        <div className="absolute -top-10 left-1/2 transform -translate-x-1/2 whitespace-nowrap bg-gray-800 text-white text-xs px-3 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20 shadow-lg">
                          OPENフォロー:{" "}
                          {item.openFollowStartDate.toLocaleDateString("ja-JP", { month: "numeric", day: "numeric" })}{" "}
                          〜 {item.openFollowEndDate.toLocaleDateString("ja-JP", { month: "numeric", day: "numeric" })}
                        </div>
                      </div>
                    )}
                  </div>
          ...
          <div className="mt-8 p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg border border-gray-200">
            <div className="flex gap-8 justify-center items-center text-sm">
              ...
              <div className="text-xs text-gray-500 ml-4">※ 工事期間・OPEN期間をクリックして基準日を変更できます</div>
            </div>
          </div>
        </div>
      </div>
